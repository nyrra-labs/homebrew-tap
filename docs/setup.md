...
- upstream `nyrra-signals` and `shpitdev/foundry-cli` release workflows can also trigger this workflow automatically with `gh workflow run version-bumps.yml`, but that depends on `NYRRA_WORKFLOW_DISPATCH_TOKEN` being configured in those producer repos
...
Store that PAT as the GitHub Actions secret `NYRRA_WORKFLOW_DISPATCH_TOKEN` in these producer repos:
...
```bash
cd /path/to/shpitdev/foundry-cli
...
```bash
depot ci secrets add NYRRA_WORKFLOW_DISPATCH_TOKEN --repo shpitdev/foundry-cli
